...
static bool
queue_over_write(espnow_msg_id_t msg_id, const void *const data, size_t data_len, void *arg, TickType_t xTicksToWait)
{
  if (msg_id == ESPNOW_EVENT_RECV_ACK) {
    ...
    espnow_event_ctx_t espnow_event = { .msg_id = msg_id, .data_len = data_len, .data = (void *) data, .handle = arg };
...
void
espnow_recv_cb(const esp_now_recv_info_t *recv_info, const uint8_t *data, int size)
#else
void
espnow_recv_cb(const uint8_t *addr, const uint8_t *data, int size)
#endif
{
  ...
  ESP_LOGD(TAG,
           "[%s, %d], " MACSTR ", rssi: %d, size: %d, total: %d - %d, type: %d, addr: %02x, g_msg_magic_cache_next: %d",
           __func__,
           __LINE__,
           MAC2STR(addr),
           rx_ctrl->rssi,
  ...
  if (espnow_data->version != ESPNOW_VERSION || (espnow_data->type >= ESPNOW_DATA_TYPE_MAX) ||
      size != espnow_data->size + sizeof(espnow_data_t) || ESPNOW_ADDR_IS_SELF(espnow_data->src_addr)) {
    ESP_LOGD(TAG,
             "Receive cb args error, recv_addr: " MACSTR ", src_addr: " MACSTR ", data: %p, size: %d",
             MAC2STR(addr),
             MAC2STR(espnow_data->src_addr),
             data,
             size);
    return;
  }
  ...
  if (g_recv_handle[espnow_data->type].enable && espnow_data->type != ESPNOW_DATA_TYPE_ACK &&
      espnow_data->type != ESPNOW_DATA_TYPE_GROUP && frame_head->ack && ESPNOW_ADDR_IS_SELF(espnow_data->dest_addr)) {
    ...
    if (!g_espnow_queue || queue_over_write(ESPNOW_EVENT_SEND_ACK,
                                            ack_data,
                                            sizeof(espnow_data_t),
                                            NULL,
  ...
  for (size_t i = 0, index = g_msg_magic_cache_next; i < ESPNOW_MSG_CACHE;
    ...
    if (g_msg_magic_cache[index].type == espnow_data->type && g_msg_magic_cache[index].magic == frame_head->magic) {
      return;
    }
  }
  ...
  if (espnow_data->type == ESPNOW_DATA_TYPE_ACK) {
    ...
    ESP_LOGD(TAG,
             ">[%s, %d]: broadcast: %d, dest_addr: " MACSTR,
             __func__,
             __LINE__,
             frame_head->broadcast,
             MAC2STR(espnow_data->dest_addr));
    ...
    *magic          = frame_head->magic;
    ...
    ESP_LOGD(TAG,
             ">1.2< group_id: " MACSTR ", dest_addr: " MACSTR,
             MAC2STR(group_info->group_id),
             MAC2STR(espnow_data->dest_addr));
    ESP_LOGD(TAG, ">1.2< addr_mun: %d, dest_addr: " MACSTR, group_info->addrs_num, MAC2STR(group_info->addrs_list[0]));
    ...
    if (!frame_head->group && frame_head->broadcast && !ESPNOW_ADDR_IS_BROADCAST(espnow_data->dest_addr) &&
      ...
      ESP_LOGD(TAG,
               "[%s, %d]: group_num: %d, group_id: " MACSTR,
               __func__,
               __LINE__,
               espnow_get_group_num(),
               MAC2STR(espnow_data->dest_addr));
      goto FORWARD_DATA;
    }
...
void
espnow_send_cb(const uint8_t *addr, esp_now_send_status_t status)
{
  if (g_buffered_num) {
    g_buffered_num--;
  }
...
esp_err_t
espnow_add_peer(const espnow_addr_t addr, const uint8_t *lmk)
{
...
esp_err_t
espnow_del_peer(const espnow_addr_t addr)
{
  ESP_PARAM_CHECK(addr);
...
static esp_err_t
espnow_send_process(int count, espnow_data_t *espnow_data, uint32_t wait_ticks, bool *ack)
{
  ...
  if (g_buffered_num >= MAX_BUFFERED_NUM || frame_head->ack || !frame_head->broadcast) {
    ...
    EventBits_t uxBits = xEventGroupWaitBits(g_event_group,
                                             SEND_CB_OK | SEND_CB_FAIL,
                                             pdTRUE,
                                             pdFALSE,
                                             MIN(wait_ticks, g_espnow_config->send_max_timeout));
...
esp_err_t
espnow_send(espnow_data_type_t type,
            const espnow_addr_t dest_addr,
            const void *data,
            size_t size,
            const espnow_frame_head_t *data_head,
            TickType_t wait_ticks)
{
  ...
  if (g_espnow_config->sec_enable && type != ESPNOW_DATA_TYPE_ACK && type != ESPNOW_DATA_TYPE_FORWARD &&
    ...
    ret               = espnow_sec_auth_encrypt(g_espnow_sec,
                                  data,
                                  size,
                                  espnow_data->payload,
                                  size + g_espnow_sec->tag_len,
                                  &enc_len,
                                  g_espnow_sec->tag_len);
  ...
  ESP_LOGD(TAG,
           "[%s, %d] addr: " MACSTR ", size: %d, count: %d, rssi: %d, data: %s, magic: 0x%x",
           __func__,
           __LINE__,
           MAC2STR(dest_addr),
           espnow_data->size,
           frame_head->retransmit_count,
           frame_head->forward_rssi,
           espnow_data->payload,
           frame_head->magic);
  ...
  for (int count = 0; count < frame_head->retransmit_count; ++count) {
    for (int i = 0;
         i == 0 || (g_set_channel_flag && frame_head->channel == ESPNOW_CHANNEL_ALL && i < g_self_country.nchan);
         ++i) {
        ...
        if (ret == ESP_OK) {
          ...
          write_ticks = (wait_ticks == portMAX_DELAY) ? portMAX_DELAY
                        : xTaskGetTickCount() - start_ticks < wait_ticks
                          ? wait_ticks - (xTaskGetTickCount() - start_ticks)
                          : 0;
      ...
      ESP_ERROR_CONTINUE(ret != ESP_OK,
                         "[%s, %d] <%s> esp_now_send, channel: %d",
                         __func__,
                         __LINE__,
                         esp_err_to_name(ret),
                         g_self_country.schan + i);
    }
  }
...
EXIT:
  ...
  if (frame_head->ack && !ESPNOW_ADDR_IS_BROADCAST(dest_addr)) {
    ...
    while (g_ack_queue &&
           xQueueReceive(g_ack_queue, &ack_magic, MIN(write_ticks, g_espnow_config->send_max_timeout)) == pdPASS) {
      if (*ack_magic == frame_head->magic) {
        ESP_FREE(espnow_data);
        ESP_FREE(ack_magic);
        return ESP_OK;
...
esp_err_t
espnow_set_group(const uint8_t addrs_list[][ESPNOW_ADDR_LEN],
                 size_t addrs_num,
                 const uint8_t group_id[ESPNOW_ADDR_LEN],
                 espnow_frame_head_t *data_head,
                 bool type,
                 TickType_t wait_ticks)
{
  esp_err_t ret        = ESP_OK;
  ...
  espnow_data_t *espnow_data =
    ESP_MALLOC(addrs_num > 35 ? ESPNOW_PAYLOAD_LEN
                              : sizeof(espnow_data_t) + sizeof(espnow_group_info_t) + addrs_num * ESPNOW_ADDR_LEN);
  espnow_frame_head_t *frame_head = &espnow_data->frame_head;
  espnow_group_info_t *group_info = (espnow_group_info_t *) espnow_data->payload;
  ...
  for (int i = 0; addrs_num > 0; ++i) {
    ...
    for (int count = 0; count < frame_head->retransmit_count; ++count) {
      for (int i = 0;
           i == 0 || (frame_head->channel == ESPNOW_CHANNEL_ALL && i < g_self_country.nchan && g_set_channel_flag);
           ++i) {
...
static esp_err_t
espnow_recv_process(espnow_pkt_t *q_data)
{
  ...
  ESP_LOGD(TAG,
           "[%s, %d], " MACSTR ", magic: 0x%04x, type: %d, size: %d, %s",
           __func__,
           __LINE__,
           MAC2STR(espnow_data->src_addr),
           frame_head->magic,
    ...
    if (frame_head->security) {
      if (g_espnow_config->sec_enable) {
        if (g_espnow_sec && g_espnow_sec->state == ESPNOW_SEC_OVER) {
          ret = espnow_sec_auth_decrypt(g_espnow_sec,
                                        espnow_data->payload,
                                        espnow_data->size,
                                        data,
                                        ESPNOW_PAYLOAD_LEN,
...
static esp_err_t
espnow_send_forward(espnow_data_t *espnow_data)
{
  ESP_PARAM_CHECK(espnow_data);
  ESP_ERROR_RETURN(!g_espnow_config, ESP_ERR_ESPNOW_NOT_INIT, "ESPNOW is not initialized");
  ...
  ESP_LOGD(TAG,
           "[%s, %d], " MACSTR ", total: %d, type: %d, magic: 0x%x",
           __func__,
           __LINE__,
           MAC2STR(espnow_data->src_addr),
           espnow_data->size,
  ...
  for (int count = 0; !count || count < frame_head->retransmit_count; ++count) {
    for (int i = 0; i == 0 || (frame_head->channel == ESPNOW_CHANNEL_ALL && i < g_self_country.nchan &&
                               g_set_channel_flag && g_espnow_config->forward_switch_channel);
         ++i) {
...
esp_err_t
espnow_init(const espnow_config_t *config)
{
...
esp_err_t
espnow_set_config_for_data_type(espnow_data_type_t type, bool enable, handler_for_data_t handle)
{
  ESP_ERROR_RETURN(!g_espnow_config, ESP_ERR_ESPNOW_NOT_INIT, "ESPNOW is not initialized");
...
esp_err_t
espnow_get_config_for_data_type(espnow_data_type_t type, bool *enable)
{
  ESP_PARAM_CHECK(type >= ESPNOW_DATA_TYPE_ACK && type < ESPNOW_DATA_TYPE_MAX);
...
esp_err_t
espnow_set_key(uint8_t key_info[APP_KEY_LEN])
{
...
esp_err_t
espnow_get_key(uint8_t key_info[APP_KEY_LEN])
{
  ESP_PARAM_CHECK(key_info);
