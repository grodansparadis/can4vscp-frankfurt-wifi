...
#define LED_INDICATOR_CHECK(a, str, ret)                                                                               \
  if (!(a)) {                                                                                                          \
...
static SLIST_HEAD(_led_indicator_head_t, _led_indicator_slist_t)
  s_led_indicator_slist_head = SLIST_HEAD_INITIALIZER(s_led_indicator_slist_head);

static esp_err_t
_led_indicator_add_node(_led_indicator_t *p_led_indicator)
{
...
static esp_err_t
_led_indicator_remove_node(_led_indicator_t *p_led_indicator)
{
  LED_INDICATOR_CHECK(p_led_indicator != NULL, "pointer can not be NULL", ESP_ERR_INVALID_ARG);
 ...
 */
static esp_err_t
_led_set_state(int io_num, bool off_level, blink_step_state_t state)
{
  uint32_t level = 0;
 ...
 */
static void
_blink_list_switch(_led_indicator_t *p_led_indicator)
{
 ...
 */
static void
_blink_list_runner(TimerHandle_t xTimer)
{
...
led_indicator_create(int io_num, const led_indicator_config_t *config)
{
  ...
  LED_INDICATOR_CHECK_GOTO(p_led_indicator->p_blink_steps != NULL,
                           "calloc blink_steps memory failed",
                           cleanup_indicator);
  ...
  switch (p_led_indicator->mode) {
    case LED_GPIO_MODE: /**< blink with max brightness*/
    {
      ...
      p_led_indicator->h_timer =
        xTimerCreate(timmer_name, (pdMS_TO_TICKS(100)), pdFALSE, (void *) p_led_indicator, _blink_list_runner);
...
led_indicator_handle_t
led_indicator_get_handle(int io_num)
{
  _led_indicator_slist_t *node;
  SLIST_FOREACH(node, &s_led_indicator_slist_head, next)
  {
...
esp_err_t
led_indicator_delete(led_indicator_handle_t *p_handle)
{
...
esp_err_t
led_indicator_start(led_indicator_handle_t handle, led_indicator_blink_type_t blink_type)
{
  LED_INDICATOR_CHECK(handle != NULL && blink_type >= 0 && blink_type < BLINK_MAX,
                      "invalid p_handle",
                      ESP_ERR_INVALID_ARG);
...
esp_err_t
led_indicator_stop(led_indicator_handle_t handle, led_indicator_blink_type_t blink_type)
{
  LED_INDICATOR_CHECK(handle != NULL && blink_type >= 0 && blink_type < BLINK_MAX,
                      "invalid p_handle",
                      ESP_ERR_INVALID_ARG);
